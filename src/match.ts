...
export async function runMatch(
  ...
  console.log(`Starting matching: ${fromEditionId} -> ${toEditionId}, limit: ${limit}`);
  ...
  if (fromSegments.length === 0) {
    console.log('No segments to match');
    return;
  }
  ...
  for (const fromSeg of fromSegments) {
    try {
      await matchSegment(fromSeg, toEditionId);
      processed++;
      if (processed % 10 === 0 || processed === fromSegments.length) {
        console.log(`Progress: ${processed}/${fromSegments.length} (${errors} errors)`);
      }
    } catch (error) {
      errors++;
